...
METHOD_DIR = DL_ROOT / "Tecnicas_registration" / "01_voxelmorph"
...
def dice_loss_from_logits(warped_mask: torch.Tensor, fixed_mask: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
...
class SmallVoxelMorph2D(nn.Module):
    def __init__(self, max_disp_norm: float = 0.22):
        super().__init__()
...
def run(
    ...
    model = SmallVoxelMorph2D(max_disp_norm=max_disp_norm).to(device)
    ...
    for step in range(iterations + 1):
        ...
        overlap_loss = dice_loss_from_logits(torch.clamp(warped_mask, 0, 1), fixed_mask_t)
    ...
    output_dir = METHOD_DIR / "outputs" / f"outputs_voxelmorph_instance{suffix}_{subject}"
    output_dir.mkdir(parents=True, exist_ok=True)

    Image.fromarray(warped_rgb).save(output_dir / f"{subject}_he_voxelmorph_instance_to_hsi.png")
    Image.fromarray((warped_mask.astype(np.uint8) * 255)).save(output_dir / f"{subject}_he_mask_voxelmorph_instance_to_hsi.png")
    np.savez_compressed(output_dir / f"{subject}_voxelmorph_instance_flow_px.npz", flow_px=flow_px)

    overlay = contour_overlay(data["fixed_rgb"], warped_rgb, data["fixed_mask"], warped_mask)
    Image.fromarray(overlay).save(output_dir / f"{subject}_overlay_contours_voxelmorph_instance.png")
    ...
    axes[1].set_title(f"VoxelMorph-style | Dice={final_dice:.3f}")
    axes[1].axis("off")
    ...
    fig.tight_layout()
    fig.savefig(output_dir / f"{subject}_voxelmorph_instance_summary.png", dpi=180)
    plt.close(fig)

    summary = {
        "subject": subject,
        "method": "VoxelMorph-style instance-specific 2D CNN",
        "device": str(device),
    ...
    with open(output_dir / f"{subject}_voxelmorph_instance_summary.json", "w", encoding="utf-8") as f:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Instance-specific VoxelMorph-style registration.")
    parser.add_argument("--subject", default="SB013")
    parser.add_argument("--iterations", type=int, default=400)
    parser.add_argument("--size", type=int, default=256)
    parser.add_argument("--lr", type=float, default=2e-3)
    parser.add_argument("--smooth-weight", type=float, default=0.08)
    parser.add_argument("--dice-weight", type=float, default=0.45)
    parser.add_argument("--max-disp-norm", type=float, default=0.22)
